...
const SignIn = function() {
    ...
    const submit = async () => {
        ...
        if (password.length < 6 || password.length > 10) {
            setErrorMessage("Password should be greater or equal to 6 and less than or equal to 10 characters long.")
            return
        }
        ...
        try {
            ...
            localStorage.setItem("refresh_token", data.token_pair.refresh_token.token_string)
            setErrorMessage("")
        ...
        <div className='flex-center'>
            <div className='signin-root'>
                ...
                <div className='signin-switch-signin-method'>
                    <button className='signin-switch-signin-button' onClick={() => {
                        if (useEmail) {
                            setEmail('')
                        }
                        else {
                    ...
                    }}>{useEmail ? "Use user name" : "Use email"}</button>
                </div>
                <div className='signin-error-message'>{errorMessage}</div>
                <button className='signin-submit' onClick={submit}>Sign In</button>
            </div>
        </div>
    )
}
